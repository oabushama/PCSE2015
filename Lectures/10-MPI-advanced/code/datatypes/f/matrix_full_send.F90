...
program datatype
  ...
  tag1 = 1; dest = 1; src = 0
  if(rank == 0)then
    write(*,'(/,2(a,i0),a,/)')"Completing Send/Recv from ",src," to ",dest,"..."
  end if
  if (rank == 0) then
    call MPI_SEND(matrix,NROWS*NCOLS,MPI_DOUBLE_PRECISION,dest, &
    &             tag1,MPI_COMM_WORLD,ierr)
  end if
  if (rank == 1) then
    src = 0; dest = 0
    call MPI_RECV(matrix,NROWS*NCOLS,MPI_DOUBLE_PRECISION,src, &
