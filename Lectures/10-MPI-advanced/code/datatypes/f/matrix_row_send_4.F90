...
! Temporary contiguous copy could go out of scope with blocking calls 
...
program datatype
  ...
  if (rank == 0) then
    call MPI_SEND(matrix(row,:),NCOLS,MPI_DOUBLE_PRECISION,dest, & ! NO NO NO
    &             tag1,MPI_COMM_WORLD,ierr)
  end if
  if (rank == 1) then
    call MPI_RECV(matrix(row,:),NCOLS,MPI_DOUBLE_PRECISION,src, & ! NO NO NO
