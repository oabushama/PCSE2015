! SCARY WARNING BAD Incorrect attempt at sending one row of a 2D array
! Temporary contiguous copy goes out of scope with non-blocking calls!
program datatype
  use mpi
  implicit none
  ...
  if (rank == 0) then
    call MPI_ISEND(matrix(row,:),NCOLS,MPI_DOUBLE_PRECISION,dest, & ! NO NO NO
    &              tag1,MPI_COMM_WORLD,reqs(1),ierr)
    call MPI_WAIT(reqs(1),stats(1,1),ierr)
  end if
  if (rank == 1) then
    call MPI_IRECV(matrix(row,:),NCOLS,MPI_DOUBLE_PRECISION,src, & ! NO NO NO
    &              tag1,MPI_COMM_WORLD,reqs(2),ierr)
